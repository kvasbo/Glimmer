...
class PageKretsVelger extends React.Component {
  constructor(props) {
    ...
    _getDataArray() {
      ...
      for (let i = 0; i < this.props.krets.length; i++) {
        krets.push(this.props.users[this.props.krets[i]]);
      }

      krets.sort((x, y) => x.name.toLowerCase().localeCompare(y.name.toLowerCase()));
...
export default connect(mapStateToProps)(PageKretsVelger);
