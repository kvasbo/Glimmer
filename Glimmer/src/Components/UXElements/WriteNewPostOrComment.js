...
export default class WriteNewPostOrComment extends React.Component {
    ...
    tick() {
      if (this._isMounted) {
        ...
        const editEnd = new moment(this.props.comment.updated_at).add(14, 'm');
        const diff = editEnd.diff(now, 's');
    ...
    _post() {
      ...
      if (!this.props.edit && this.props.type === 'comment') {
        ...
        const forumId = store.getState().AppStatus.activePostingForum;
    ...
    insertIntoBodyText(text) {
      ...
      if (this.state.bodyCursorPosition === null && this.state.text === '') this.setState({ text });

      const tmpText = this.state.text;
      const start = tmpText.substring(0, this.state.bodyCursorPosition);
      const tail = tmpText.substring(this.state.bodyCursorPosition, tmpText.length);
      if (this._isMounted) {
    ...
    getImageListContent() {
      ...
      for (const key in this.state.images) {
        outImg.push(<TouchableOpacity key={key} onPress={() => this.insertIntoBodyText(`!${this.state.images[key].uri}!`)}>
          <View style={{
justifyContent: 'flex-start', margin: 10, width: 75, height: 75,
}}
          >
            <Image
              source={{ uri: this.state.images[key].orig_uri }}
              style={{
height: 75, width: 75, position: 'absolute', top: 0, left: 0, borderRadius: 0,
}}
            />
    ...
    getLoadingIndicator(loading) {
      if (loading) {
        return (
          <View style={{
 backgroundColor: '#FFFFFF66', position: 'absolute', top: 0, left: 0, height: 75, width: 75, padding: 20,
}}
          >
            <ActivityIndicator size="large" color="#444444" hidesWhenStopped />
          </View>
        );
      }
    }
    ...
    _getTimer() {
      ...
      if (this.state.timeRemaining < 0) {
        ...
        const t = new moment(this.props.comment.updated_at);

        t.add(14, 'm');

        const fromNow = t.fromNow(true);
    ...
    _uploadAndAddPicture(pictureData) {
      ...
      try {
        ...
        const extension = filenameArr.slice(-1)[0];
