...
export default class WriteNewPostOrComment extends React.Component {
    ...
    getLoadingIndicator(loading) {
      if (loading) {
        return (
          <View style={{
backgroundColor: '#FFFFFF66', height: 75, width: 75, padding: 20,
}}
          ><ActivityIndicator
            size="large"
            color="#444444"
    ...
    getImageList() {
      ...
      for (const key in this.state.images) {
        outImg.push(<View key={key} style={{ justifyContent: 'flex-start', margin: 10 }}>
          <TouchableOpacity
          ...
          >
            <Image
              ...
              style={{
 height: 75, width: 75, margin: 5, borderRadius: 0,
}}
            >
    ...
    _uploadAndAddPicture(pictureData) {
      ...
      try {
        ...
        const filename = pictureData.filename.toLowerCase();
    ...
    _getImagePickerModal() {
      ...
      for (let i = 0; i < this.state.photos.length; i++) {
        out.push(<TouchableOpacity
          key={this.state.photos[i].filename}
          onLongPress={() => this._uploadAndAddPicture(this.state.photos[i])}
        >
          <Image
            source={{ uri: this.state.photos[i].uri }}
            resizeMode="contain"
