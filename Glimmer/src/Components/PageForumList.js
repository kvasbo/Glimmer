...
export default class PageForumList extends React.Component {
    ...
    componentWillMount() {
      ...
      this.reduxUnsubscribe = store.subscribe(() => {
        const newForum = store.getState().AppStatus.activePostingForum;
    ...
    getFilterList() {
      ...
      for (const key in this.filters.special) {
        out.push(<View key={this.filters.special[key]}>
          <TouchableOpacity
            onPress={() => this.setFilter(this.filters.special[key])}
          >
            <Text style={[pageStyles.forumSelection, pageStyles.selectionSpecial, { color: this.getFilterColor(this.filters.special[key]) }]}>{this.filters.special[key]}
            </Text>
          </TouchableOpacity>
                 </View>);
      }

      for (const key in this.filters.alpha) {
        out.push(<View key={this.filters.alpha[key]}>
          <TouchableOpacity
            onPress={() => this.setFilter(this.filters.alpha[key])}
          >
            <Text style={[pageStyles.forumSelection, pageStyles.selectionLetter, { color: this.getFilterColor(this.filters.alpha[key]) }]}>{this.filters.alpha[key]}
            </Text>
          </TouchableOpacity>
        </View>);
      }

      for (const key in this.filters.theEnd) {
        out.push(<View key={this.filters.theEnd[key]}>
          <TouchableOpacity
            onPress={() => this.setFilter(this.filters.theEnd[key])}
          >
            <Text style={[pageStyles.forumSelection, pageStyles.selectionLetter, { color: this.getFilterColor(this.filters.theEnd[key]) }]}>{this.filters.theEnd[key]}
            </Text>
          </TouchableOpacity>
        </View>);
      }
    ...
    getFilteredForumList() {
      ...
      arr.sort((x, y) => x.title.trim().localeCompare(y.title.trim()));

      if (this.state.filter === 'populære') {
        const muchUsed = []; //= this.state.muchUsedForums;
        // Lag array av mye brukte
        for (const key in this.state.muchUsedForums) {
          muchUsed.push(this.state.muchUsedForums[key]);
        }

        arr = arr.filter(x => (muchUsed.includes(x.id)));

        return arr;
      } else if (this.state.filter === '#') {
        return arr.filter(forum => !this.filters.alpha.includes(forum.title.toLocaleLowerCase().substring(0, 1)));
      }

      return arr.filter(forum => forum.title.toLocaleLowerCase().indexOf(this.state.filter.toLocaleLowerCase()) === 0);
    }

    _renderItem(item) {
      const selected = (item.id === this.state.chosenForum);
      return (<Forum forum={item} selected={selected} />);
    }
...
class Forum extends React.Component {
    ...
    render() {
      const style = (this.props.selected) ? this.styles.selectedText : this.styles.forumText;
