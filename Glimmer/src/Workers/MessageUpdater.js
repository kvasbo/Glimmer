    ...
    return new Promise((resolve, reject) => {
      ...
      api.makeApiGetCall(uri).then((result) => {
        ...
        for (let i = 0; i < result.data.length; i++) {
          ...
          const n = new Message(m.id, m.body, m.sent_at, m.dismissed_at, m.from.id, m.from.name, m.from.image_url, m.to.id, m.to.name, m.to.image_url);
          messages.push(n);
        }
    ...
    return new Promise((resolve, reject) => {
      this.getMessageThreads(page).then((data) => {
        ...
        for (const key in data) {
          ...
          const c = data[key];
          const convo = new Conversation(c.user.name, c.user.id, c.message_count, c.unread_count, c.user.image_url, c.last_message.from.id, c.last_message.sent_at, c.last_message.body);
